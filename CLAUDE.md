...
This is a **.NET 8 Aspire** microservices solution with 6 projects:
...
- **ApiCore** — Business API (currently WeatherForecast). Protected by JWT. Uses its own `coredb` PostgreSQL database.
- **Web** — Blazor Server frontend. Authenticates via cookie session server-side, stores JWT in `localStorage` client-side via `Blazored.LocalStorage`. Calls both APIs through typed HTTP clients.
- **ServiceDefaults** — Shared extension methods applied to all services: OpenTelemetry, service discovery, HTTP resilience.
- **Tests** — Aspire integration tests using `DistributedApplicationTestingBuilder`.

### Authentication Flow
1. User POSTs credentials to `ApiIdentity /auth/login` → receives a JWT (1-hour expiry)
2. Web stores JWT in `localStorage` via `Blazored.LocalStorage`
3. `JwtAuthStateProvider` reads the token and provides `AuthenticationState` to Blazor
4. Typed HTTP clients attach the JWT as a Bearer token when calling `ApiCore`
...
- **PostgreSQL 17.6** container with two databases
- **Redis** container used for output caching in the Web project
- Docker Compose generation configured via `.aspire/` and `infra/` directories
